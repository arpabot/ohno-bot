...
import { channels, users } from "./cache.js";
...
export default function cleanContent(
  ...
  return message.content
    ...
    .replaceAll(discordPattern, (...args: unknown[]) => {
      ...
      if (
        !__<{ guildId: string; channelId: string; messageId?: string }>(groups)
      )
        return "";
    ...
    .replaceAll(uriPattern, (...args: unknown[]) => {
      ...
      if (!__<UriPatternGroup>(groups)) return "";
...
function generateReplacer(
  ...
  return (...args: unknown[]) => {
    ...
    if (!__<{ id: string; name: string }>(groups)) return "";

    return `@${converter(groups) || fallback}`;
  };
}
function __<T>(x: unknown): x is T {
  return x != null;
}
